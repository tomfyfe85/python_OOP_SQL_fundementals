...
def parse_records(raw_records: list[dict]) -> list[dict]:
    ...
    for current_record in raw_records:
        if all(current_record[score].isnumeric() for score in ["home_score", "away_score"]): 
            current_record["home_score"] = int(current_record["home_score"])
            current_record["away_score"] = int(current_record["away_score"])
